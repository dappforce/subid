...
const StatusCard = ({ data, key, status, className }: Props) => {
  ...
  return (
    <div key={key} className={clsx(styles.StatusCol, className)}>
      <Card
        className={clsx(styles.StatusCard, { [styles.Disconnected]: !status })}
      >
        <ChainData
          name={name}
          icon={icon}
...
const StatusSection = ({
  ...
  const sectionTitle = (
    <h2 className={clsx({ ['ml-3']: isMobile })}>
      {title} networks: {statuses.length} / {supportedNetworks.length}
    </h2>
  )
  ...
  const statusItems = statuses.map(({ status, network }, index) => {
    ...
    return <StatusCard key={index} data={chainInfoByNetwork} status={status} />
  })
  ...
  return (
    <div className={className}>
      ...
      <div className={styles.StatusCardsGrid}>{statusItems}</div>
    </div>
  )
}
