...
import { FormInstance, Button, Form, FormItemProps, Input, InputProps } from 'antd'
...
export function TokenAmountInput ({ disableMaxButton, onMaxClick, ...props }: TokenAmountInputProps) {
  const { t } = useTranslation()
  const maxBtn = onMaxClick && (
    <Button disabled={disableMaxButton} type='link' className='font-weight-semibold pr-2' onClick={onMaxClick}>
      {t('transfer.max')}
    </Button>
  )

  return (
    <Input {...props} min='0' step='0.1' type='number' suffix={maxBtn} />
  )
}
...
export function TokenAmountFormItem ({
  ...
  const { token, sourceChain, destChain } = getTokenData(getToken, getSourceChain, getDestChain)
  const { formattedTransferableBalance: currentBalance, tokenDecimal } = useTransferableBalance(token, sourceChain)
  const { fee, loading } = useFetchTransferFee({ source: sourceChain, dest: destChain, token })
...
const existentialDepositLink = 'https://support.polkadot.network/support/solutions/articles/65000168651-what-is-the-existential-deposit-#:~:text=On%20the%20Polkadot%20network%2C%20an,the%20Existential%20Deposit%20(ED)'
...
function ExistentialDepositAlert ({
  ...
  const { formattedTransferableBalance: currentBalance, existentialDeposit: sourceExistentialDeposit = 0 } = useTransferableBalance(token, sourceChain)
  const { existentialDeposit: destExistentialDeposit = 0 } = useTransferableBalance(token, destChain || sourceChain)
  ...
  const insufficientSourceExistential = currentBalance - currentAmount < sourceExistentialDeposit
  const showExistentialDepositHelper = insufficientDestExistential || insufficientSourceExistential

  const getAlertDest = () => {
    const network = destChainInfo?.name || sourceChainInfo?.name
    const existential = `${destExistentialDeposit} ${token}`
    const minTransfer = `${toShortMoney({ num: totalFeeAndDeposit, fractions: 4 })} ${token}`
    return (
      <Trans t={t} values={{ token, existential, network, minTransfer }} i18nKey='transfer.existentialDepositInDestAlert'>
        The <a target='_blank' rel='noreferrer noopener' href={existentialDepositLink}>existential deposit</a> for token on network is existential. Please transfer more than minTransfer.
      </Trans>
    )
  }
  const getAlertSource = () => {
    ...
    return (
      <Trans t={t} values={{ token, network, existential }} i18nKey='transfer.existentialDepositInSourceAlert'>
        The <a target='_blank' rel='noreferrer noopener' href={existentialDepositLink}>existential deposit</a> for token on network is existential. Please leave at least existential or transfer the entire balance to prevent your account from being reaped.
      </Trans>
    )
  }
