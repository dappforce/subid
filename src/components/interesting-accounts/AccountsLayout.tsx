...
import {
  getAccountDataByChainNameAndType,
  MenuItems,
  CHAIN_ITEMS,
  parseCrowdloanersData,
} from './utils'
...
const AccountsLayout = ({
  ...
  const [ selectedChain, setSelectedChain ] = useState<string>(
    (chain as string) || DEFAULT_SELECTED_CHAIN
  )
  ...
  const loadMore = createLoadMoreInterestingAccountsFn(
    fetchFunction,
    selectedChain,
    tabKey,
    chainInfo,
  ...
  const selectedChainLabel =
    selectedChain === 'all' ? t('chains.allChains') : capitalize(selectedChain)
  const roleLabel =
    tabKey === 'all'
      ? t('interestingAccounts.allRoles')
      : t(`interestingAccounts.${tabKey}`)

  const List = useCallback(() => {
    const itemClassName =
      tabKey !== 'all' && tabKey !== 'crowdloaner'
        ? styles.ShortCardDesc
        : undefined

    const renderItem = (account: AccountCardType) => (
      <AccountCard accountData={account} className={itemClassName} />
    )
    return (
      <InfinitePageList
        withLoadMoreLink
  ...
  return (
    <div>
      ...
      <Row
        ...
        className={clsx({ ['pl-3 pr-3']: isMobile })}
      >
        <Col>
          <div className={styles.DropdownContainer}>
            {isMobile ? (
              ...
              >
                <MutedSpan>
                  {roleLabel} <DownOutlined className='ml-1 FontNormal' />
                </MutedSpan>
              </Dropdown>
            ) : (
        ...
        >
          <Dropdown
            ...
            overlay={
              <MenuItems
                onTabChange={onSelectedChainChange}
                data={CHAIN_ITEMS}
                dropdownType='Chains'
                withIcon
              />
          ...
          >
            <div className={styles.SelectedNetwork}>
              ...
              <MutedSpan>
                {selectedChainLabel} <DownOutlined className='ml-1' />
              </MutedSpan>
            </div>
          </Dropdown>
        </Col>
      ...
      {isMobile && (
        <div className='d-flex justify-content-center'>
          <Divider className={styles.AccountsDivider} />
        </div>
      )}
