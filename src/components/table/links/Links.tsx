...
const LinkButtonsOverlay = ({
  ...
  const actionMenuItems = actionMenuItemsValuesEntries
    .map(([ fieldName, values ], i) => {
      ...
      return (
        <Menu.Item
          key={`${fieldName}-${i}`}
          disabled={fieldName === 'actionTransfer' && disableTransferButton}
        >
          {menuItem}
        </Menu.Item>
      )
  ...
  const socialMenuItems = socialMenuItemsValuesEntries
    .map(([ fieldName, values ], i) => {
      const { label, icon } = values || {}
  ...
  return (
    <Menu selectable={false}>
      {actionMenuItems.length && (
        <>
          {actionMenuItems}
          <div className={styles.MoreButtonDividor}>
            <Divider />
          </div>
        </>
      )}
